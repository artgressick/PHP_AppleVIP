<?	
	...
	$info = fetch_database_query("SELECT I.ID, I.bType, I.idStatus, I.intGuests, I.idUser, I.idShow, C.ID AS idContact, C.chrFirst AS chrContactFirst, C.chrLast AS chrContactLast, C.chrCompany, 
									...
									WHERE I.ID='".$_REQUEST['id']."'","Getting Invite Information");
	if($info['ID'] == '') {
		$_SESSION['errorMessages'][] = "Invalid Contact";
		$moveTo = 'invites.php?'.$_SESSION['inviteRefer'];
		unset($_SESSION['inviteRefer']);
	...
	$q = "SELECT Invites.ID, Invites.intGuests
			...
			WHERE Invites.idShow='". $info['idShow'] ."' AND Invites.idUser='". $info['idUser'] ."' AND Invites.idStatus IN (2,5,6,8,9) AND !Contacts.bDeleted";
	...
	if(isset($_POST['Save'])) {
		$q = "SELECT Invites.ID, Invites.intGuests
				FROM Invites
				JOIN Contacts ON Invites.idContact=Contacts.ID
				WHERE Invites.idShow='". $info['idShow'] ."' AND Invites.idUser='". $info['idUser'] ."' AND Invites.ID != '".$info['ID']."' AND Invites.idStatus IN (2,5,6,8,9) AND !Contacts.bDeleted";
		...
		$_SESSION['postUser'] = $info['PostUser'];
		$moveTo = 'invites.php?'.$_SESSION['inviteRefer'];
		unset($_SESSION['inviteRefer']);
		header("Location: ".$moveTo);	
		die();
