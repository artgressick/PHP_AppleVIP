			...
			<div class='warning' id='warning'>
				...
				<div class='body'>
					<div>You are about to remove: <br />
						...
						<input type='hidden' value='' id='chrTablePrefix' name='chrTablePrefix' />
					</div>
					<div style='margin-top: 20px; '><strong>Are you sure you want to do this? It cannot be undone!</strong><br />
						<input type='button' value='Yes' onclick="javascript:delItem('<?=$BF?>ajax_delete.php?postType=permdelete&tbl=<?=$TableName?>&idUser=<?=$_SESSION['idUser']?>&id='); document.idForm.submit();" /> &nbsp;&nbsp; <input type='button' value='No' onclick="javascript:revert();" />
					</div>
				</div>
			</div>
		</div>
